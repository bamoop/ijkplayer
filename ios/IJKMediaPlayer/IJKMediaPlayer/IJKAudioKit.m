...
- (void)setupAudioSession
{
    ...
    if (NO == [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayback error:&error]) {
        DNSLog(@"IJKAudioKit: AVAudioSession.setCategory() failed: %@\n", error ? [error localizedDescription] : @"nil");
        return;
    }

    error = nil;
    if (NO == [[AVAudioSession sharedInstance] setActive:YES error:&error]) {
        DNSLog(@"IJKAudioKit: AVAudioSession.setActive(YES) failed: %@\n", error ? [error localizedDescription] : @"nil");
        return;
    }
...
- (BOOL)setActive:(BOOL)active
{
    if (active != NO) {
        ...
        @try {
            ...
            DNSLog(@"failed to inactive AVAudioSession\n");
        }
    }
}
...
- (void)handleInterruption:(NSNotification *)notification
{
    ...
    switch (reason) {
        case AVAudioSessionInterruptionTypeBegan: {
            DNSLog(@"AVAudioSessionInterruptionTypeBegan\n");
            [self setActive:NO];
            break;
        }
        case AVAudioSessionInterruptionTypeEnded: {
            DNSLog(@"AVAudioSessionInterruptionTypeEnded\n");
            [self setActive:YES];
            break;
        }
    }
